...
select *
from libri l inner join huazimi h on l.ID = h.LibriiHuazuarId
...
select SUM(l.qmimi) as shuma,l.DataPranimit as shuma
from libri l where (l.DataPranimit>='2019-01-01' and l.DataPranimit<='2021-12-31');
...
select emri , mbiemri ,LibriiHuazuarId,zhanri
from ((biblioteka b inner join lexuesi l on l.ID=b.LexuesiId) inner join libri li on li.Id=b.libriId)
inner join huazimi h  on h.LibriiHuazuarId=l.Id where h.vonesa>0;
# 8. Gjeni top librin me te lexuar per vitin aktual.
select max(l.NumriIkopjeve) from libri l inner join biblioteka b where l.Id=b.libriId and b.viti=now();
# 9. Cili eshte autori me i preferuar nga lexuesit?
select Autori , max(l.NumriIkopjeve) from libri l;
